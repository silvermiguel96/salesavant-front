...
export const normalize_country_name = (country_name) => {
  if (country_name) {
    return country_name.trim().toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "");
  }
};

const build_country_index = () => {
  let res = {};
  for (const [key, value] of Object.entries(COUNTRY_CODES)) {
    let key_norm = normalize_country_name(key);
    if (key_norm) {
      res[key_norm] = value;
    }
  }
...
const COUNTRY_INDEX = build_country_index();

export const find_country_code = (country_name) => {
  let country_name_norm = normalize_country_name(country_name);
  if (country_name_norm in COUNTRY_INDEX) {
    return COUNTRY_INDEX[country_name_norm];
  }
};